package handlers

import (
	"github.com/KMHTelU/KMH-WebProfile-API/internal/requests"
	"github.com/KMHTelU/KMH-WebProfile-API/utils"
	"github.com/gofiber/fiber/v3"
	"github.com/google/uuid"
)

func (h *Handler) CreateEventHandler(c fiber.Ctx) error {
	var req requests.CreateEventRequest
	if err := c.Bind().JSON(&req); err != nil {
		errorsMap := utils.MapValidationErrors(err)
		if errorsMap != nil {
			return utils.RespondWithValidationError(c, errorsMap)
		}
		return utils.RespondWithError(c, fiber.StatusBadRequest, "Bad request")
	}
	err := h.Service.CreateEventService(req, c)
	if err != nil {
		return err
	}
	return utils.RespondWithCreated(c, "Event created successfully", nil)
}

func (h *Handler) GetEventByIDHandler(c fiber.Ctx) error {
	id := utils.GetSingleParams(c)
	if id == uuid.Nil {
		return utils.RespondWithError(c, fiber.StatusBadRequest, "Invalid ID")
	}
	event, err := h.Service.GetEventByIDService(id, c)
	if err != nil {
		return utils.RespondWithError(c, err.Code, err.Message)
	}
	return utils.RespondWithOK(c, "Event retrieved successfully", event)
}

func (h *Handler) GetPaginatedEventsHandler(c fiber.Ctx) error {
	limit, offset := utils.GetPaginationParams(c)
	events, err := h.Service.GetPaginatedEventsService(limit, offset, c)
	if err != nil {
		return utils.RespondWithError(c, err.Code, err.Message)
	}
	return utils.RespondWithOK(c, "Events retrieved successfully", events)
}

func (h *Handler) UpdateEventHandler(c fiber.Ctx) error {
	id := utils.GetSingleParams(c)
	if id == uuid.Nil {
		return utils.RespondWithError(c, fiber.StatusBadRequest, "Invalid ID")
	}
	var req requests.UpdateEventRequest
	if err := c.Bind().JSON(&req); err != nil {
		errorsMap := utils.MapValidationErrors(err)
		if errorsMap != nil {
			return utils.RespondWithValidationError(c, errorsMap)
		}
		return utils.RespondWithError(c, fiber.StatusBadRequest, "Bad request")
	}
	err := h.Service.UpdateEventService(id, req, c)
	if err != nil {
		return err
	}
	return utils.RespondWithOK(c, "Event updated successfully", nil)
}

func (h *Handler) DeleteEventHandler(c fiber.Ctx) error {
	id := utils.GetSingleParams(c)
	if id == uuid.Nil {
		return utils.RespondWithError(c, fiber.StatusBadRequest, "Invalid ID")
	}
	err := h.Service.DeleteEventService(id, c)
	if err != nil {
		return err
	}
	return utils.RespondWithOK(c, "Event deleted successfully", nil)
}
